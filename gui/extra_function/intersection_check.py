...
# Pij为直线i的第j个端点，linei = [x0,y0,x1,y1]
def is_line_line_intersection(line0: structs.Line, line1: structs.Line):
    ...
    if (
            max(p0.x, p1.x) >= min(p2.x, p3.x)  # 矩形1最右端大于矩形2最左端
            and max(p2.x, p3.x) >= min(p0.x, p1.x)  # 矩形2最右端大于矩形1最左端
            and max(p0.y, p1.y) >= min(p2.y, p3.y)  # 矩形1最高端大于矩形2最低端
            and max(p2.y, p3.y) >= min(p0.y, p1.y)  # 矩形2最高端大于矩形1最低端
    ):
        # 若通过快速排斥则进行跨立实验
...
# 矩形和直线相交：直线在矩形内或是矩形的任意一边在直线上，rect=[左下，右上]
